...
?><!doctype html>
<html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?= htmlspecialchars($seoTitle, ENT_QUOTES, 'UTF-8') ?></title><meta name="description" content="<?= htmlspecialchars($seoDescription, ENT_QUOTES, 'UTF-8') ?>"><?php if ($seoKeywords !== ''): ?><meta name="keywords" content="<?= htmlspecialchars($seoKeywords, ENT_QUOTES, 'UTF-8') ?>"><?php endif; ?><link rel="canonical" href="<?= htmlspecialchars($seoCanonical, ENT_QUOTES, 'UTF-8') ?>"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet"><link rel="stylesheet" href="/assets/css/core/shared-typography.css"><style>
...
<main class="facility-page site-typo"><section class="container">
  <nav class="breadcrumb"><a href="/">Trang chủ</a><span>›</span><strong>Toplist</strong></nav>
  <div class="hero-head"><div><div class="hero-title-row"><h1>Toplist cơ sở y tế</h1><span class="title-pill"><?= count($rows) ?> bài viết</span></div><p class="hero-sub">Các danh sách tổng hợp giúp bạn so sánh và lựa chọn cơ sở phù hợp.</p></div><div class="hero-note"><span class="icon"><i data-lucide="shield-check"></i></span><div><strong>Danh sách được biên tập</strong><span>Thông tin cơ sở được tổng hợp rõ ràng để bạn dễ đối chiếu.</span></div></div></div>
  <div class="filter-bar"><div class="filter-item"><label>Tìm kiếm</label><div class="filter-input"><span>Tìm bài viết Toplist...</span><i data-lucide="search"></i></div></div><div class="filter-item"><label>Sắp xếp</label><div class="filter-select"><span>Mới cập nhật</span><i data-lucide="chevron-down"></i></div></div></div>
  <div class="content-grid"><div class="list-wrap"><?php foreach($rows as $index=>$row): ?><article class="facility-card"><div class="facility-media"><?php if(trim((string)$row['featured_image_url']) !== ''): ?><img src="<?= htmlspecialchars($row['featured_image_url'],ENT_QUOTES) ?>" alt="<?= htmlspecialchars($row['title'],ENT_QUOTES) ?>"><?php endif; ?><span class="media-count"><i data-lucide="list-ordered"></i> Toplist</span></div><div class="facility-main"><div class="facility-title"><span class="rank-badge"><?= $index+1 ?></span><h2><?= htmlspecialchars($row['title'],ENT_QUOTES) ?></h2></div><p class="facility-sub"><?= htmlspecialchars((string)($row['excerpt'] ?: 'Khám phá danh sách cơ sở y tế được chọn lọc.'),ENT_QUOTES) ?></p><div class="meta-row"><i data-lucide="calendar-days"></i><span>Cập nhật <?= htmlspecialchars(date('d/m/Y',strtotime((string)$row['updated_at'])),ENT_QUOTES) ?></span></div></div><div class="score-col"><div class="score-main"><?= (int)$row['facility_count'] ?></div><div class="score-meta">cơ sở trong danh sách</div></div><div class="cta-col"><strong>Khám phá danh sách</strong><a class="detail-btn" href="/toplist-chi-tiet-mau.php?slug=<?= rawurlencode($row['slug']) ?>">Xem chi tiết</a></div></article><?php endforeach; ?></div><aside class="sidebar"><section class="summary-card"><strong>Tổng quan Toplist</strong><div class="summary-number"><?= count($rows) ?></div><p>Bài viết toplist đã xuất bản, cập nhật theo dữ liệu cơ sở y tế trên MedReview.</p></section><section class="utility-card"><strong>Tiện ích</strong><div class="utility-list"><div class="utility-item"><span class="utility-icon"><i data-lucide="scale"></i></span><div><strong>So sánh nhanh</strong><p>Đối chiếu nhiều cơ sở trong cùng bài viết.</p></div></div><div class="utility-item"><span class="utility-icon"><i data-lucide="shield-check"></i></span><div><strong>Thông tin có chọn lọc</strong><p>Xem nội dung và danh sách cơ sở rõ ràng.</p></div></div></div></section></aside></div>
</section></main>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.facility-card').forEach(card => {
      ...
      images.forEach(source => {
        ...
        image.src = source;
        image.alt = 'Ảnh cơ sở trong Toplist';
