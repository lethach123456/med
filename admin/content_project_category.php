...
if ($id <= 0) {
    flash_toast_set('danger', 'Thiếu chuyên mục dự án.', 'fa-solid fa-triangle-exclamation');
    header('Location: /admin/content.php');
    exit;
}

$stmt = $pdo->prepare('SELECT id, name, slug FROM project_categories WHERE id = :id LIMIT 1');
$stmt->execute([':id' => $id]);
$category = $stmt->fetch();
if (!$category) {
...
$itemsStmt = $pdo->prepare(
    "SELECT id, title, slug, featured_image_url, location, year, status, created_at, updated_at
     FROM projects
...
$adminPageTitle = 'Admin • Chuyên mục dự án';
$adminHeaderTitle = 'Chuyên mục dự án';
      ...
      <div class="card-body p-4">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
          ...
          <div class="d-flex gap-2">
            ...
            <a class="btn btn-outline-primary" href="/admin/content_project_category_edit.php?id=<?php echo $id; ?>"><i class="fa-solid fa-pen-to-square me-2" aria-hidden="true"></i>Sửa chuyên mục</a>
            <a class="btn btn-primary" href="/admin/content_project_edit.php?category_id=<?php echo $id; ?>"><i class="fa-solid fa-plus me-2" aria-hidden="true"></i>Thêm dự án</a>
          </div>
        </div>
          ...
          <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
              <tr>
                ...
                <th>Dự án</th>
                <th style="width:180px;">Vị trí / Năm</th>
            ...
            <tbody>
              <?php if (!is_array($items) || count($items) === 0): ?>
                <tr><td colspan="6" class="text-secondary">Chưa có dự án trong chuyên mục này.</td></tr>
              <?php else: ?>
                  ...
                  <tr>
                    ...
                    <td class="text-secondary small">
                      <?php
                        ...
                        $year = (string) ($p['year'] ?? '');
                        echo htmlspecialchars(trim($loc . ($loc !== '' && $year !== '' ? ' • ' : '') . $year), ENT_QUOTES, 'UTF-8');
                      ?>
                    </td>
                    <td>
                    ...
                    <td class="text-end">
                      <div class="d-inline-flex gap-2">
                        <a class="btn btn-sm btn-outline-secondary" target="_blank" href="/du-an/<?php echo htmlspecialchars(rawurlencode((string) ($p['slug'] ?? '')), ENT_QUOTES, 'UTF-8'); ?>"><i class="fa-solid fa-arrow-up-right-from-square me-2" aria-hidden="true"></i>Xem</a>
                        <a class="btn btn-sm btn-outline-primary" href="/admin/content_project_edit.php?id=<?php echo (int) $p['id']; ?>"><i class="fa-solid fa-pen-to-square me-2" aria-hidden="true"></i>Sửa</a>
                        <button class="btn btn-sm btn-outline-danger" type="button" data-delete-type="project" data-delete-id="<?php echo (int) $p['id']; ?>"><i class="fa-solid fa-trash me-2" aria-hidden="true"></i>Xoá</button>
                      </div>
                    </td>
                  </tr>
                <?php endforeach; ?>
