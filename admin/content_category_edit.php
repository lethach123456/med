...
if ($isEdit) {
    $stmt = $pdo->prepare('SELECT id, name, slug, language, description, seo_title, seo_description FROM categories WHERE id = :id LIMIT 1');
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch();
    if (!$row) {
        if ($isModal) {
            json_response(['ok' => false, 'message' => 'Chuyên mục không tồn tại.'], 404);
        }
...
function render_category_modal_form(array $values, array $errors, bool $isEdit, int $id): string
{
    ob_start();
    ?>
    <div class="p-2 p-sm-0">
      ...
      <form method="post" action="/admin/content_category_edit.php?modal=1" class="row g-3" data-modal-form="1">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($values['name'] === '') {
        $errors[] = 'Vui lòng nhập tên chuyên mục.';
    }
    ...
    $values['slug'] = unique_slug($pdo, 'categories', $slugInput, $isEditPost ? $postId : null);
    ...
    if (count($errors) === 0) {
        if ($isEditPost) {
            ...
            if ($isModal) {
                json_response(['ok' => true, 'message' => 'Đã lưu chuyên mục.', 'id' => $postId]);
            }
            flash_toast_set('success', 'Đã lưu chuyên mục.', 'fa-solid fa-circle-check');
            header('Location: /admin/content_category_edit.php?id=' . $postId);
            exit;
        }

        $stmt = $pdo->prepare(
            'INSERT INTO categories (name, slug, language, description, seo_title, seo_description)
        ...
        if ($isModal) {
            json_response(['ok' => true, 'message' => 'Đã tạo chuyên mục.', 'id' => $newId]);
        }
        flash_toast_set('success', 'Đã tạo chuyên mục.', 'fa-solid fa-circle-check');
        header('Location: /admin/content_category_edit.php?id=' . $newId);
        exit;
    }
    ...
    if ($isModal) {
        json_response([
            ...
            'html' => render_category_modal_form($values, $errors, $isEditPost, $postId),
        ], 422);
    }
}

if ($isModal) {
    echo render_category_modal_form($values, [], $isEdit, $id);
    exit;
}

$adminPageTitle = $isEdit ? 'Admin • Sửa chuyên mục' : 'Admin • Thêm chuyên mục';
$adminHeaderTitle = $isEdit ? 'Sửa chuyên mục' : 'Thêm chuyên mục';
      ...
      <div class="card-body p-4">
        <div class="d-flex align-items-center justify-content-between gap-2 mb-3">
          ...
          <div class="d-flex gap-2">
            ...
            <?php if ($isEdit): ?>
              <a class="btn btn-outline-primary" href="/admin/content_category.php?id=<?php echo $id; ?>"><i class="fa-solid fa-list me-2" aria-hidden="true"></i>Quản lý trang</a>
            <?php endif; ?>
          </div>
        </div>
