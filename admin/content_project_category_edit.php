...
if ($isEdit) {
    $stmt = $pdo->prepare('SELECT id, name, slug, language, description, seo_title, seo_description FROM project_categories WHERE id = :id LIMIT 1');
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch();
    if (!$row) {
        if ($isModal) {
            json_response(['ok' => false, 'message' => 'Chuyên mục dự án không tồn tại.'], 404);
        }
        flash_toast_set('danger', 'Chuyên mục dự án không tồn tại.', 'fa-solid fa-triangle-exclamation');
...
function render_project_category_modal_form(array $values, array $errors, bool $isEdit, int $id): string
{
    ob_start();
    ?>
    <div class="p-2 p-sm-0">
      ...
      <form method="post" action="/admin/content_project_category_edit.php?modal=1" class="row g-3" data-modal-form="1">
        ...
        <div class="col-12">
          <label class="form-label" for="name">Tên chuyên mục dự án</label>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($values['name'] === '') {
        $errors[] = 'Vui lòng nhập tên chuyên mục dự án.';
    }
    ...
    $values['slug'] = unique_slug($pdo, 'project_categories', $slugInput, $isEditPost ? $postId : null);
    ...
    if (count($errors) === 0) {
        if ($isEditPost) {
            ...
            if ($isModal) {
                json_response(['ok' => true, 'message' => 'Đã lưu chuyên mục dự án.', 'id' => $postId]);
            }
            flash_toast_set('success', 'Đã lưu chuyên mục dự án.', 'fa-solid fa-circle-check');
            header('Location: /admin/content_project_category_edit.php?id=' . $postId);
            exit;
        }

        $stmt = $pdo->prepare(
            'INSERT INTO project_categories (name, slug, language, description, seo_title, seo_description)
        ...
        if ($isModal) {
            json_response(['ok' => true, 'message' => 'Đã tạo chuyên mục dự án.', 'id' => $newId]);
        }
        flash_toast_set('success', 'Đã tạo chuyên mục dự án.', 'fa-solid fa-circle-check');
        header('Location: /admin/content_project_category_edit.php?id=' . $newId);
        exit;
    }
    ...
    if ($isModal) {
        json_response([
            ...
            'html' => render_project_category_modal_form($values, $errors, $isEditPost, $postId),
        ], 422);
    }
}

if ($isModal) {
    echo render_project_category_modal_form($values, [], $isEdit, $id);
    exit;
}

$adminPageTitle = $isEdit ? 'Admin • Sửa chuyên mục dự án' : 'Admin • Thêm chuyên mục dự án';
$adminHeaderTitle = $isEdit ? 'Sửa chuyên mục dự án' : 'Thêm chuyên mục dự án';
      ...
      <div class="card-body p-4">
        <div class="d-flex align-items-center justify-content-between gap-2 mb-3">
          ...
          <div class="d-flex gap-2">
            ...
            <?php if ($isEdit): ?>
              <a class="btn btn-outline-primary" href="/admin/content_project_category.php?id=<?php echo $id; ?>"><i class="fa-solid fa-list me-2" aria-hidden="true"></i>Quản lý dự án</a>
            <?php endif; ?>
          </div>
        </div>
