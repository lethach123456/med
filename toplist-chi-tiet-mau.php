...
$toplistNotFound = !is_array($toplist);
if (!$toplistNotFound && parse_url((string) ($_SERVER['REQUEST_URI'] ?? ''), PHP_URL_PATH) === '/toplist-chi-tiet-mau.php') {
    $rowLocale = strtolower((string) ($toplist['language_code'] ?? 'vi')) === 'en' ? 'en' : 'vi';
    medical_redirect_legacy_path('/toplist-chi-tiet-mau.php', medical_public_entity_path('toplist', $incomingToplistSlug, $rowLocale), ['slug']);
}
if ($toplistNotFound) { http_response_code(404); $toplist = ['id'=>0,'title'=>'Không tìm thấy bài viết','excerpt'=>'Bài Toplist chưa tồn tại hoặc chưa được xuất bản.','content'=>'','featured_image_url'=>'','updated_at'=>date('Y-m-d H:i:s')]; }
$toplistLanguage = strtolower((string) ($toplist['language_code'] ?? 'vi')) === 'en' ? 'en' : 'vi';
$toplistLanguageLinks = !$toplistNotFound
    ? medical_directory_translation_switch_links($pdo, 'toplist', $toplist)
...
$toplistSchema = [
    ...
    'dateModified' => date(DATE_ATOM, strtotime((string) ($toplist['updated_at'] ?? 'now')) ?: time()),
...
ob_start(static function (string $html) use ($facilities): string {
    ...
    $script .= '<script>document.addEventListener("DOMContentLoaded",()=>{const galleries='.json_encode($toplistFacilityGalleries, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES).';document.querySelectorAll(".facility").forEach(card=>{const href=card.querySelector(".detail-btn")?.getAttribute("href")||"",slug=new URL(href,location.origin).searchParams.get("slug")||"",images=galleries[slug]||[],cover=card.querySelector(".facility-cover");if(!cover||images.length<2)return;const strip=document.createElement("div");strip.className="toplist-gallery-strip";images.slice(0,6).forEach(src=>{const button=document.createElement("button"),image=document.createElement("img");button.type="button";button.className="toplist-gallery-thumb";image.src=src;image.alt="Ảnh cơ sở";image.loading="lazy";image.decoding="async";button.append(image);button.addEventListener("click",()=>{const main=cover.querySelector("img");if(main){const current=main.src;main.src=image.src;image.src=current;}});strip.append(button);});cover.after(strip);});});</script>';
...
HTML;
    ...
    // Defer below-the-fold images while keeping the toplist hero as the LCP image.
    $html = str_replace(
        '<div class="hero-image"><img ',
        '<div class="hero-image"><img loading="eager" fetchpriority="high" decoding="async" ',
        $html
    );
    $html = preg_replace_callback('~<img\\b([^>]*?)>~i', static function (array $match): string {
        $attributes = $match[1];
        if (stripos($attributes, 'loading=') !== false) return $match[0];
