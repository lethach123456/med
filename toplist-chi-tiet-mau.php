...
HTML;
    ...
    $css .= '<style>.toplist-quick-compare + .toplist-quick-compare{display:none}</style>';
    ...
    $script .= '<script>document.addEventListener("DOMContentLoaded",()=>{const rows='.json_encode($quickComparisonRows, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES).';const main=document.querySelector(".main"),anchor=main?.querySelector(".intro")||main?.querySelector(".section-head");if(!main||!anchor||!rows.length)return;const esc=value=>String(value??"").replace(/[&<>\"]/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[char]));const section=document.createElement("section");section.className="toplist-quick-compare";section.innerHTML=`<div class="toplist-quick-compare-head"><div><h2>So sánh nhanh các cơ sở</h2><p>Đối chiếu dịch vụ, giá tham khảo và đánh giá trước khi xem hồ sơ chi tiết.</p></div></div><div class="toplist-quick-compare-scroll"><table><thead><tr><th>Cơ sở</th><th>Dịch vụ nổi bật</th><th>Giá tham khảo</th><th>Điểm · số lượng đánh giá</th></tr></thead><tbody>${rows.map(row=>`<tr><td><span class="quick-rank">${esc(row.rank)}</span><span class="quick-name">${esc(row.name)}</span></td><td><div class="quick-services">${(row.services||[]).map(service=>`<span>${esc(service)}</span>`).join("")||"—"}</div></td><td>${esc(row.price||"Liên hệ cơ sở")}</td><td class="quick-score"><b>★ ${esc(row.rating)}</b>/5 · ${esc(row.reviews)}</td></tr>`).join("")}</tbody></table></div>`;anchor.after(section);});</script>';
