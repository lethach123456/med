...
$seo = front_editor_page_seo('co-so-y-te', [
  ...
  'description' => 'Khám phá và so sánh các cơ sở y tế, phòng khám và nha khoa được đánh giá thực tế trên MedReview.',
  'canonical_path' => '/co-so-y-te.php',
]);
$title = (string) ($seo['title'] ?? 'Cơ sở y tế • MedReview');
$description = (string) ($seo['description'] ?? '');
$canonicalPath = (string) ($seo['canonical_path'] ?? '/co-so-y-te.php');
$seoKeywords = (string) ($seo['keywords'] ?? '');

$filters = [
  'search' => 'Tìm theo tên nha khoa...',
  'region' => 'Tất cả khu vực',
  'service' => 'Tất cả dịch vụ',
  'rating' => 'Tất cả đánh giá',
  'price' => '0đ - 100.000.000đ+',
  'sort' => 'Mới nhất',
];

$facilities = medical_directory_facility_rows(true);
foreach ($facilities as &$facilityItem) {
  $facilityItem = medical_directory_facility_with_linked_reviews($facilityItem, true);
  $summary = (array) ($facilityItem['review_summary'] ?? []);
  if (isset($summary['rating'])) $facilityItem['rating'] = (string) $summary['rating'];
  if (isset($summary['reviews'])) $facilityItem['reviews'] = (string) $summary['reviews'];
  $gallery = is_array($facilityItem['gallery'] ?? null) ? $facilityItem['gallery'] : [];
  if (trim((string) ($facilityItem['image'] ?? '')) === '' && $gallery !== []) {
    $facilityItem['image'] = (string) $gallery[0];
  }
  if (trim((string) ($facilityItem['images'] ?? '')) === '' && $gallery !== []) {
    $facilityItem['images'] = count($gallery) . ' ảnh';
  }
}
unset($facilityItem);
...
$utilities = [
  ['icon' => 'scale', 'title' => 'So sánh nha khoa', 'text' => 'Chọn và so sánh nhiều cơ sở'],
  ['icon' => 'bookmark', 'title' => 'Lưu nha khoa yêu thích', 'text' => 'Lưu để xem sau'],
  ['icon' => 'shield-check', 'title' => 'Gợi ý nha khoa phù hợp', 'text' => 'Dựa trên nhu cầu của bạn'],
];
?>
<!doctype html>
<html lang="vi">
  <head>
    ...
    <style>
      ...
      *{box-sizing:border-box}
      ...
      .facility-page{padding:22px 0 64px}
      .breadcrumb{
        display:flex;
        align-items:center;
        gap:10px;
      ...
      .facility-card{
        display:grid;
        grid-template-columns:195px minmax(0,1.7fr) 104px 152px 134px 122px;
        gap:14px;
        padding:8px 10px;
        border:1px solid var(--border);
        border-radius:18px;
      ...
      .media-count svg{width:14px;height:14px;stroke-width:2}
      .facility-main{padding:6px 0}
      .facility-title{
        display:flex;
        align-items:center;
        gap:10px;
      ...
      .rank-badge{
        width:26px;height:26px;border-radius:999px;background:linear-gradient(180deg,#3b82f6,#2563eb);color:#fff;
        display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;flex:0 0 auto;
      }
      .facility-title h3{margin:0;font-size:1.1em;line-height:1.25}
      .verified{
      ...
      .facility-sub{
        margin:4px 0 0;color:var(--muted);font-size:11px;line-height:1.55;max-width:36ch;
      }
      .facility-meta{
        margin-top:10px;
        display:grid;
      ...
      .meta-row{
        display:flex;align-items:flex-start;gap:7px;color:#475569;font-size:12px;line-height:1.5;
      }
      .meta-row svg{width:15px;height:15px;stroke-width:2;flex:0 0 auto;margin-top:2px;color:#94a3b8}
      .score-col,
      .service-col,
      .price-col,
      .cta-col{
      ...
      .score-main small{font-size:.45em;color:#64748b}
      .stars{
        display:flex;
        gap:2px;
        margin-top:8px;
      ...
      .score-meta{margin-top:4px;color:#64748b;font-size:10px;line-height:1.55}
      .service-col strong,
      .price-col strong,
      .summary-card strong,
      .customer-box strong{
      ...
      .tag{
        display:inline-flex;align-items:center;justify-content:center;padding:5px 9px;border-radius:999px;
        background:#f5f7fb;color:#475569;font-size:10px;font-weight:600;
      }
      .price-main{
        color:var(--brand);
        font-size:.97em;
      ...
      .cta-col{display:grid;gap:12px;align-content:start}
      .detail-btn{
      ...
      @media (max-width:1240px){
        ...
        .facility-card{grid-template-columns:180px minmax(0,1fr) 110px 1fr}
        .price-col,.cta-col{grid-column:auto}
      }
      @media (max-width:980px){
        ...
        .facility-media{height:220px}
        .score-col,.service-col,.price-col,.cta-col{padding-top:0}
      }
      @media (max-width:720px){
        .container{width:min(100% - 24px, var(--max))}
        .facility-page{padding:20px 0 48px}
  ...
  <body>
    ...
    <main class="facility-page site-typo">
      <section class="container">
        <nav class="breadcrumb" aria-label="Breadcrumb">
          <a href="/">Trang chủ</a>
          <span class="crumb-sep" aria-hidden="true"><i data-lucide="chevron-right"></i></span>
          <a href="/danh-muc-y-te.php">Nha khoa</a>
          <span class="crumb-sep" aria-hidden="true"><i data-lucide="chevron-right"></i></span>
          <span class="active">Đánh giá nha khoa Đà Nẵng</span>
        </nav>

        <div class="hero-head">
          <div>
            <div class="hero-title-row">
              <h1>Đánh giá nha khoa Đà Nẵng</h1>
              <span class="title-pill"><i data-lucide="badge-check"></i> 658 đánh giá thực tế</span>
            </div>
            <p class="hero-sub">Xem đánh giá, trải nghiệm thực tế từ khách hàng để lựa chọn nha khoa phù hợp nhất.</p>
          </div>
          <div class="hero-note">
            <span class="icon"><i data-lucide="shield-check"></i></span>
            <div>
              <strong>Tất cả đánh giá đều từ khách hàng thật</strong>
              <span>Thông tin minh bạch - khách quan - không quảng cáo</span>
            </div>
          </div>
        </div>
            ...
            <?php foreach ($facilities as $item): ?>
              <article class="facility-card">
                <div class="facility-media">
                  ...
                  <span class="media-count"><i data-lucide="image"></i><?php echo htmlspecialchars($item['images'], ENT_QUOTES, 'UTF-8'); ?></span>
                </div>

                <div class="facility-main">
                  <div class="facility-title">
                    <span class="rank-badge"><?php echo (int) $item['rank']; ?></span>
                  ...
                  <p class="facility-sub"><?php echo htmlspecialchars($item['subtitle'], ENT_QUOTES, 'UTF-8'); ?></p>
                  <div class="facility-meta">
                    <div class="meta-row"><i data-lucide="clock-3"></i><span><?php echo htmlspecialchars($item['hours'], ENT_QUOTES, 'UTF-8'); ?></span></div>
                  </div>
                </div>

                <div class="score-col">
                  <div class="score-main"><?php echo htmlspecialchars($item['rating'], ENT_QUOTES, 'UTF-8'); ?><small>/5</small></div>
                  <div class="stars rating-stars-text" aria-label="<?php echo htmlspecialchars($item['rating'], ENT_QUOTES, 'UTF-8'); ?> trên 5">★★★★★</div>
                  <div class="score-meta">
                    <div><?php echo htmlspecialchars($item['reviews'], ENT_QUOTES, 'UTF-8'); ?></div>
                    <div><?php echo htmlspecialchars($item['followers'], ENT_QUOTES, 'UTF-8'); ?></div>
                  </div>
                </div>
                ...
                <div class="service-col">
                  ...
                  <div class="tag-list">
                    <?php foreach (array_slice((array) $item['services'], 0, 4) as $service): ?>
                      <span class="tag"><?php echo htmlspecialchars($service, ENT_QUOTES, 'UTF-8'); ?></span>
                    <?php endforeach; ?>
                    <?php if (count((array) $item['services']) > 4): ?><span class="tag tag-more">+<?php echo count((array) $item['services']) - 4; ?></span><?php endif; ?>
                  </div>
                </div>

                <div class="price-col">
                  <strong>Khoảng giá</strong>
                  <div class="price-main"><?php echo htmlspecialchars($item['price'], ENT_QUOTES, 'UTF-8'); ?></div>
                </div>

                <div class="cta-col">
                  <strong>Tổng quan</strong>
                  <a class="detail-btn" href="/co-so-y-te-chi-tiet.php?slug=<?php echo rawurlencode((string) $item['slug']); ?>">Xem chi tiết</a>
                </div>
              </article>
            <?php endforeach; ?>
          </div>
          ...
          <aside class="sidebar">
            <section class="summary-card">
              ...
              <div class="summary-top">
                <div>
                  ...
                  <div class="summary-caption">658 đánh giá</div>
                </div>
              </div>
            ...
            <section class="customer-box">
              <div class="customer-head">
                ...
                <div>
                  <strong>Bạn là khách hàng?</strong>
                  <p>Chia sẻ trải nghiệm để giúp mọi người chọn đúng nha khoa.</p>
                </div>
              </div>
        ...
        <p class="page-foot">Dữ liệu được cập nhật mô phỏng theo ảnh mẫu. Thông tin chỉ mang tính minh họa giao diện, bạn có thể thay trực tiếp trong file sau.</p>
      </section>
    </main>
